...
from awcp_instrumentation import InstrumentationResult, run_instrumentation

_log = logging.getLogger(__name__)


class InstrumentationError(RuntimeError):
    """Raised by start() when the governance pre-flight check blocks the launch."""
...
def start(agent: dict) -> None:
    ...
    control plane. Non-blocking (Popen returns immediately).

    Raises:
        InstrumentationError: if the governance pre-flight check fails.
            The agent is NOT launched in this case.
    """
    # Governance pre-flight: run instrumentation before launching the agent.
    try:
        result: InstrumentationResult = run_instrumentation(agent["dir"])
    except Exception as exc:
        raise InstrumentationError(
            f"Pre-flight error for agent '{agent['id']}': {type(exc).__name__}: {exc}"
        ) from exc

    if not result.success:
        _log.warning(
            "Instrumentation pre-flight BLOCKED agent '%s'. %s",
            agent["id"], result.repository_summary,
        )
        raise InstrumentationError(
            f"Agent '{agent['id']}' did not pass governance instrumentation. "
            f"{result.repository_summary}"
        )

    _log.info(
        "Instrumentation pre-flight PASSED for agent '%s'. %s",
        agent["id"], result.repository_summary,
    )
