...
from awcp.gateway.agents_fs import InstrumentationError
    ...
    if not state["running"] or not state["port"]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=424,
                detail={
                    "message": f"agent '{agent_id}' blocked by governance instrumentation",
                    "reason": str(exc),
                    "hint": "All required AWCP hooks must pass sandbox validation before an agent can launch.",
                },
            )
