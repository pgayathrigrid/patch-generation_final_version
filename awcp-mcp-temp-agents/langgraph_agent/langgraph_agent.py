...
def run_goal(goal: str) -> dict:
    ...
    get_manager().dispatch(HookType.BUDGET_WARN, agent_id="langgraph", task_id=None, used_ratio=0.8)
    get_manager().dispatch(HookType.TASK_COMPLETED, agent_id="agent", task_id=None)
    ...
    get_manager().dispatch(HookType.LLM_CALL, agent_id="awcp_agent", task_id=None, model=MODEL)
    get_manager().dispatch(HookType.TASK_FAILED, agent_id="LangGraph Orchestrator", task_id=None, error=None)
