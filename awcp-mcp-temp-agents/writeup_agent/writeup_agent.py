...
def run_goal(goal: str) -> dict:
    get_manager().dispatch(HookType.TOKEN_USAGE, agent_id=AGENT_NAME, task_id=None, prompt_tokens=getattr(res.usage, 'prompt_tokens', 0), completion_tokens=getattr(res.usage, 'completion_tokens', 0))
    get_manager().dispatch(HookType.BUDGET_WARN, agent_id=AGENT_NAME, task_id=None, used_ratio=0.8)
    get_manager().dispatch(HookType.TASK_COMPLETED, agent_id=AGENT_NAME, task_id=None)
    get_manager().dispatch(HookType.TASK_STARTED, agent_id=AGENT_NAME, task_id=None)
    get_manager().dispatch(HookType.BUDGET_EXHAUSTED, agent_id=AGENT_NAME, task_id=None, used_ratio=1.0)
    get_manager().dispatch(HookType.LLM_CALL, agent_id=AGENT_NAME, task_id=None, model=MODEL)
    get_manager().dispatch(HookType.TASK_FAILED, agent_id=AGENT_NAME, task_id=None, error="Task failed due to unhandled exception.")
