...
def run_goal(goal: str) -> dict:
    get_manager().dispatch(HookType.TOKEN_USAGE, agent_id="pydanticai_extractor", task_id=None, prompt_tokens=0, completion_tokens=0)
    get_manager().dispatch(HookType.BUDGET_WARN, agent_id="pydanticai_extractor", task_id=None, used_ratio=0.8)
    get_manager().dispatch(HookType.TASK_COMPLETED, agent_id="pydanticai_extractor", task_id=None, result=None)
    get_manager().dispatch(HookType.TASK_STARTED, agent_id="pydanticai_extractor", task_id=None, goal=goal)
    res = AGENT.run_sync(goal)
    out = getattr(res, "output", None)
    get_manager().dispatch(HookType.BUDGET_EXHAUSTED, agent_id="pydanticai_extractor", task_id=None, used_ratio=1.0)
    get_manager().dispatch(HookType.LLM_CALL, agent_id="pydanticai_extractor", task_id=None, model=MODEL)
    get_manager().dispatch(HookType.TASK_FAILED, agent_id="pydanticai_extractor", task_id=None, error="Task failed due to unhandled exception or invalid result (placeholder)")
    if out is None:
