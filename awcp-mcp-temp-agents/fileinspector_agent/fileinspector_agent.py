get_manager().dispatch(HookType.WEB_SEARCH, agent_id="file-inspector-agent", task_id=None, query="N/A - no web search capability")
...
def run_goal(goal: str) -> dict:
    ...
    get_manager().dispatch(HookType.TOKEN_USAGE, agent_id="file-inspector-agent", task_id=None, prompt_tokens=0, completion_tokens=0)
    get_manager().dispatch(HookType.BUDGET_WARN, agent_id="file-inspector-agent", task_id=None, used_ratio=0.8)
    get_manager().dispatch(HookType.TASK_COMPLETED, agent_id="file-inspector-agent", task_id=None)
    get_manager().dispatch(HookType.TASK_STARTED, agent_id="file-inspector-agent", task_id=None)
    get_manager().dispatch(HookType.BUDGET_EXHAUSTED, agent_id="file-inspector-agent", task_id=None, used_ratio=1.0)
    path = _extract_path(goal)
    get_manager().dispatch(HookType.TOOL_CALL, agent_id="file-inspector-agent", task_id=None, tool_name="classify_file", action=path)
    get_manager().dispatch(HookType.LLM_CALL, agent_id="file-inspector-agent", task_id=None, model=MODEL)
    get_manager().dispatch(HookType.TASK_FAILED, agent_id="file-inspector-agent", task_id=None, error=None)
    if not path:
